...
| Access and identity       | Mostly ready. Backend has organizations, projects, memberships, project-member create, invitations, users, user sessions, external identities, identity providers, service accounts, local single-user, generic OIDC, and CSRF-protected session mutations. API key admin lifecycle endpoints are not exposed yet.               |
...
- API key admin lifecycle endpoints
